...
const setTrumpSuit = async (redis: Redis, gameId: string, roundNumber: number, trump: string) => redis.set(`${gameId}-r${roundNumber}-trump`, trump);
...
const setCurrentRound = async (redis: Redis, gameId: string, roundNumber: number) => redis.set(
...
const setCurrentTrick = async (redis: Redis, gameId: string, trickNumber: number) => redis.set(
...
const getTrickWinners = async (redis: Redis, gameId: string, roundNumber: number) => redis.lrange(`${gameId}-r${roundNumber}-taken`, 0, -1);

const getLeadSuit = async (redis: Redis, gameId: string, roundNumber: number, trickNumber: number) => redis.get(`${gameId}-r${roundNumber}-t${trickNumber}-leadsuit`) as Promise<SUIT>;

const setLeadSuit = async (redis: Redis, gameId: string, roundNumber: number, trickNumber: number, suit: SUIT) => redis.set(`${gameId}-r${roundNumber}-t${trickNumber}-leadsuit`, suit);
...
const evaluateTrick = async (redis: Redis, gameId: string, roundNumber: number, trickNumber: number) => {
    ...
    const trickCards = cards.map((c, index) => ({ ...c, playerId: trickPlayers[index] }));
...
const setPlayerReady = async (redis: Redis, gameId: string, roundNumber: number, trickNumber: number, playerId: string) => redis.sadd(`${gameId}-r${roundNumber}-t${trickNumber}-ready`, playerId);

const getPlayersReady = async (redis: Redis, gameId: string, roundNumber: number, trickNumber: number): Promise<PlayerId[]> => redis.smembers(`${gameId}-r${roundNumber}-t${trickNumber}-ready`);
